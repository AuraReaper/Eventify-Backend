...
func (s *AuthService) Register(ctx context.Context, registerData *models.AuthCredentials) (string, *models.User, error) {
	...
	if registerData.Role != "" {
		if registerData.Role != models.Manager && registerData.Role != models.Attendee {
			return "", nil, fmt.Errorf("invalid role: must be either 'manager' or 'attendee'")
		}
	} else {
		// Set default role as attendee if not specified
		registerData.Role = models.Attendee
	}
